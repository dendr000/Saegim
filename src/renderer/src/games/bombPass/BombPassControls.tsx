...
function BombPassControls({
  question,
  activeParticipantLabel,
  onSubmit,
  onPassManually,
  onEndGame
}: BombPassControlsProps) {
  return (
    <div>
      <h3 className="stage-subtitle">지금 폭탄을 든 사람: {activeParticipantLabel}</h3>
      ...
      {question.type === 'multipleChoice' ? (
        ...
        <p className="stage-text">이 유형은 아직 폭탄 돌리기에서 지원하지 않습니다.</p>
      )}

      <div style={{ marginTop: '1rem' }}>
        <button type="button" className="stage-button stage-button-small" onClick={onPassManually}>
          폭탄 강제로 넘기기
        </button>{' '}
...
export default BombPassControls;
